...
/**
 使用在VC或View中
 self.show(text: "啦啦啦啦啦")
 self.showLoading()
 self.show(SecondViewController.init(nibName: "SecondViewController", bundle: nil), sender: nil)
 self.hideHUD()
 */
...
class SwiftLoadingTool: UIView {
    ...
    lazy var loadingTool: UIActivityIndicatorView = {
        let tool = UIActivityIndicatorView.init(style:UIActivityIndicatorView.Style.gray)
